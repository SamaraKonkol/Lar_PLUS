...
async function carregarFavoritos() {
    ...
    try {
        ...
        favoritos.forEach((imovel) => {
            favoritosIds.add(String(imovel.id));
        });
    } catch (erro) {
...
function criarCardImovel(imovel) {
    ...
    const estaFavoritado = favoritosIds.has(String(imovel.id));
    const iconeFavorito = estaFavoritado ? "♥" : "♡";
    const classeFavoritado = estaFavoritado ? " favoritado" : "";
    const textoAcessibilidade = estaFavoritado
        ? "Remover imóvel dos favoritos"
        : "Favoritar imóvel";

    return `
        <article class="card-imovel" data-id="${imovel.id}" tabindex="0" role="link" aria-label="Ver detalhes de ${imovel.titulo}">
...
function renderizarImoveis(lista) {
    listaImoveis.innerHTML = "";
    quantidadeImoveis.textContent = lista.length;
    ...
    listaImoveis.innerHTML = lista.map(criarCardImovel).join("");
}
...
function aplicarFiltros() {
    ...
    imoveisFiltrados = imoveis.filter((imovel) => {
        if (cidade && imovel.cidade !== cidade) {
            return false;
        }
        ...
        if (quartos) {
            const quantidadeQuartos = Number(imovel.quartos);

            if (quartos === "4" && quantidadeQuartos < 4) {
                return false;
            }

            if (quartos !== "4" && quantidadeQuartos !== Number(quartos)) {
                return false;
            }
        }
        ...
        if (faixaPreco === "ate-1500" && valor > 1500) {
            return false;
        }

        if (faixaPreco === "1500-2500" && (valor < 1500 || valor > 2500)) {
            return false;
        }

        if (faixaPreco === "2500-4000" && (valor < 2500 || valor > 4000)) {
            return false;
        }

        if (faixaPreco === "acima-4000" && valor <= 4000) {
            return false;
        }
...
function aplicarOrdenacao() {
    ...
    if (tipoOrdenacao === "menor-preco") {
        imoveisFiltrados.sort((a, b) => Number(a.valor) - Number(b.valor));
    }

    if (tipoOrdenacao === "maior-preco") {
        imoveisFiltrados.sort((a, b) => Number(b.valor) - Number(a.valor));
    }

    if (tipoOrdenacao === "recentes") {
        imoveisFiltrados.sort((a, b) => new Date(b.criado_em) - new Date(a.criado_em));
    }
...
async function adicionarFavorito(imovelId, botao) {
    ...
    try {
        ...
        if (!resposta.ok) {
            throw new Error(
                dados.erro || dados.mensagem || "Não foi possível favoritar o imóvel."
            );
        }

        favoritosIds.add(String(imovelId));
        botao.textContent = "♥";
        botao.classList.add("favoritado");
        botao.setAttribute("aria-label", "Remover imóvel dos favoritos");
        botao.title = "Remover imóvel dos favoritos";
    } catch (erro) {
...
async function removerFavorito(imovelId, botao) {
    ...
    try {
        ...
        if (!resposta.ok) {
            throw new Error(
                dados.erro || dados.mensagem || "Não foi possível remover o favorito."
            );
        }

        favoritosIds.delete(String(imovelId));
        botao.textContent = "♡";
        botao.classList.remove("favoritado");
        botao.setAttribute("aria-label", "Favoritar imóvel");
        botao.title = "Favoritar imóvel";
    } catch (erro) {
...
async function alternarFavorito(imovelId, botao) {
    ...
    const estaFavoritado = favoritosIds.has(String(imovelId));

    if (estaFavoritado) {
        await removerFavorito(imovelId, botao);
        return;
    }

    await adicionarFavorito(imovelId, botao);
}

function abrirDetalhesDoCard(card) {
    const imovelId = card?.dataset.id;
...
listaImoveis.addEventListener("click", async (event) => {
    const botaoFavorito = event.target.closest(".botao-favoritar");

    if (botaoFavorito) {
        event.stopPropagation();
        await alternarFavorito(botaoFavorito.dataset.id, botaoFavorito);
        return;
    }
    ...
    if (card) {
        abrirDetalhesDoCard(card);
    }
});

listaImoveis.addEventListener("keydown", (event) => {
...
async function iniciarCatalogo() {
    const carregou = await carregarImoveis();
